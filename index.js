...
function removeArtist(array, index) {
  let arrayCopytwo = [...array];
  arrayCopytwo.splice(index, 1);
  return arrayCopytwo;
}
...
function addArtist(array, nameInput, yearsInput, genreInput, nationalityInput, bioInput) {
  let arrayCopythree = [...array];
  let obj = {
    name: nameInput,
    years: yearsInput,
    genre: genreInput,
  ...
  arrayCopythree.push(obj);
  return arrayCopythree;
}
...
function artistInfo(array, nameOne){
  for (let i = 0; i < array.length; i++){
    if (nameOne === array[i].name){
      return array[i].bio;
    }
  }
}
...
function artistByCountry(array, nationalityOne){
  let artistsFrom = [];
  for (let i = 0; i < array.length; i++){
    if (nationalityOne === array[i].nationality){
      artistsFrom.push(array[i].name);
    }
  }
  return artistsFrom;
}
